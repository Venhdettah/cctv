...
class UserSwitches(AbstractInput):
    def __init__(self):
        ...
        # set pin to input
        gpio.setup(self.switches_pins["k1-pin"], gpio.IN)
        gpio.setup(self.switches_pins["k2-pin"], gpio.IN)
        gpio.setup(self.switches_pins["k3-pin"], gpio.IN)
    ...
    def start(self):
        ...
        while True:
            if gpio.input(self.switches_pins["k1-pin"]):
                logger.info("K1 button pressed")
                # send data to something

            if gpio.input(self.switches_pins["k2-pin"]):
                logger.info("K2 button pressed")
                direction = 0
                # send data to servo
                self._callback(direction)

            if gpio.input(self.switches_pins["k3-pin"]):
